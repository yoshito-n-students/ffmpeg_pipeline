...
Encoder::Encoder(const std::string &codec_name) : codec_ctx_(nullptr, &free_context) {
...
Encoder::Encoder(const CodecParameters &params) : codec_ctx_(nullptr, &free_context) {
...
void Encoder::send_frame(const Frame &frame) {
  if (const int ret = avcodec_send_frame(codec_ctx_.get(), frame.get()); ret < 0) {
...
Packet Encoder::receive_packet() {
  ...
  Packet packet;
  if (const int ret = avcodec_receive_packet(codec_ctx_.get(), packet.get());
...
std::string Encoder::codec_name() const { return avcodec_get_name(codec_ctx_->codec_id); }
...
void Encoder::free_context(AVCodecContext *codec_ctx) { avcodec_free_context(&codec_ctx); }
